<div class="w-full">
    <div class="flex items-center justify-between mb-3">
        <h2 class="font-semibold text-3xl text-gray-800 leading-tight">
            {{ __('SubCategory Listing') }}
        </h2>
        <div x-data="{ open: false }">
            <button x-on:click="open = true" class="py-2 px-4 bg-black text-white font-semibold rounded-md">Create SubCategory</button>
            <div class="fixed top-0 left-0 w-full h-full bg-center bg-white/50 backdrop-blur" x-show="open" x-transition x-cloak>
                <div class="w-full h-full flex items-center justify-center" x-on:click.self="open = false">
                    <form wire:submit='create' class="max-w-2xl p-6 rounded-lg bg-white shadow-lg w-full">
                        <h1 class="text-4xl mb-3">Create SubCategory</h1>
                        ...
                        <div class="w-full mb-3">
                            <x-input-label for="category" :value="__('Main Category')" />
    ...
    <div class="w-full">
        @if (count($categories))
            <table class="w-full">
                <tr class="bg-gray-100">
                    ...
                    <th class="text-start">MainCategory</th>
                ...
                @foreach ($categories as $category)
                    <tr class="text-sm odd:bg-gray-50 border-y border-gray-200 text-gray-500">
                        ...
                        <td class="text-start">{{ $category->category->name }}</td>
        ...
        @else
            <p class="text-center py-4">No Sub categories data found!</p>
        @endif
    </div>
</div>